...
class Game():
  ...
  def __str__(self):
    ...
    game_string = ""
    game_string += f"玩家名稱: {self.world.player.name()} ({self.world.player.current_action}), 正在前往 {self.action.walk.destination}\n"
    game_string += f"玩家 HP/SP: {self.world.player.hp()}/{self.world.player.max_hp()} | {self.world.player.sp()}/{self.world.player.max_sp()}\n"
    game_string += f"玩家狀態: {self.world.player.state()} ({self.world.player.state_map.get(self.world.player.state(), '未知')})\n"
    game_string += f"地圖名稱: {self.world.player.map_name()} {coords} {self.world.player.screen_coordinates()}\n"
    game_string += f"是否可行走? {self.map.walkable(coords)} (類型 {self.map.read_coords(coords)}) (寬度 {self.map.width}, 高度 {self.map.height})\n"
    game_string += f"是否正在與 NPC 對話? {self.world.player.is_talking_to_npc()}\n"
    game_string += f"滑鼠位置: {self.input.mouse.get_current_mouse_pos()}. 是否鎖定? {self.input.mouse.mouse_blocker.is_running()}\n"
    game_string += f"視角: {self.world.view.horizontal_camera_angle()}, {self.world.view.vertical_camera_angle()}, {self.world.view.camera_zoom()}\n"
    game_string += f"巨集啟用? {self.macro.active}\n"
    game_string += f"Bot 啟用? {self.active}\n"
    game_string += f"目前戰鬥目標: {self.action.fight.fighting_entity}\n"
